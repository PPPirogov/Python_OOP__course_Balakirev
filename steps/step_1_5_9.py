...
список объектов для покупки (объекты классов Table, TV, Notebook и Cup). Изначально этот список должен быть пустым.
...
class Cart:
    ...
    def add(self, gd):
        self.goods.append(gd)  # - добавление в корзину товара, представленного объектом gd;

    def remove(self, indx):
        del self.goods[indx]  # удаление из корзины товара по индексу indx;

    def get_list(self):
        b = []
        for a in self.goods:
            b.append(f'{a.name}: {a.price}')
        return (b)
